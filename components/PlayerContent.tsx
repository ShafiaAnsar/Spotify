...
const PlayerContent:React.FC<PlayerContentProps> = ({song,songUrl}) => {
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 h-full">
        ...
        <div className="hidden h-full md:flex justify-center items-center w-full max-w-[722px] gap-x-6">
            <AiFillStepBackward size={30 } className="cursor-pointer text-neutral-400 hover:text-white transition" onClick={onPlayBack}/>
            <div onClick={handlePlay} className="flex items-center justify-center h-10 w-10 rounded-full bg-white p-1 cursor-pointer ">
                <Icon size={30} className="text-black"/>
            </div>
            <AiFillStepForward onClick={onPlayNext} size={30 } className="cursor-pointer text-neutral-400 hover:text-white transition-black"/>
        </div>
